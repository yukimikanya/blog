...
export const getStaticPaths = async () => {
  ...
  const paths = range(1, Math.ceil(repos.totalCount / PER_PAGE)).map((repo) => `/category/d_go9idxtkw/${repo}`);
...
export const getStaticProps = async (context) => {
  ...
  const Data = await client.get({ endpoint: "blog", queries: { offset: (id - 1) * 9, limit: 9,filters: `category[equals]d_go9idxtkw` } });
  ...
  return (
    <div className='page'>
      <ul className='pn-wrapper'>
        ...
        <Link href={`/category/d_go9idxtkw/1`}><p>«</p></Link>
        </li>
        {pageNumbers.map((number, index) => (
          <li key={index} className="pn-name">
            <Link href={`/category/d_go9idxtkw/${number}`}>
              <p>{number}</p>
            </Link>
          </li>
        ))}
        <li className="pn-guide">
        <Link href={`/category/d_go9idxtkw/${totalPages}`}><p>»</p></Link>
        </li>
      </ul>
